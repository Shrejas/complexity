...
//  Complexity
//
//  Created by IE12 on 13/05/24.
//

import SwiftUI

struct DrinkNameView: View {
    @Environment(\.presentationMode) var presentationMode
    let post: FeedPost

    @StateObject  var store: LocationFeedStore
    ...
    init(post: FeedPost) {
        self.post = post
        _store = StateObject(wrappedValue: LocationFeedStore(drinkName: post.drinkName ?? ""))
    }
    ...
    var body: some View {
        VStack{
                                ...
                                ForEach($store.models) { model in
                                    ...
                                    ZStack {
                                        FeedViewCell(isLocationNameClickable: false,
                                                     ...
                                                     posts: $store.models,
                                                     post: model, isPostDeleted: $isPostDeleted,
                                                     selectedPostIdForShowLikes: $selectedPostIdForShowLikes, showLikesSheet: $showLikesSheet)
                                        { post in
            ...
            .onChange(of: isPostDeleted, perform: { newValue in
                ...
                .navigationBarTitle(Text(post.drinkName ?? "")
                                    , displayMode: .inline)
                .toolbar {
...
//#Preview {
//    DrinkNameView( store: LocationFeedStore)
//}
